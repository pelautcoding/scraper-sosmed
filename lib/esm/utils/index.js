...
export function getDecodedSnapSave(data) {
    ...
    // Check if the expected substring exists in the data
    const startSubstring = 'getElementById("download-section").innerHTML = "';
    const endSubstring = '"; document.getElementById("inputData").remove(); ';

    const startIndex = data.indexOf(startSubstring);
    const endIndex = data.indexOf(endSubstring);

    // Check if both substrings are found
    if (startIndex === -1 || endIndex === -1) {
        console.error('Error: Substrings not found in data.');
        return null; // or handle the error accordingly
    }

    // Extract the desired content
    const extractedContent = data.substring(startIndex + startSubstring.length, endIndex);

    // Remove backslashes
    const decodedContent = extractedContent.replace(/\\(\\)?/g, '');
